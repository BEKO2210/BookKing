...
import { AlertTriangle, ArrowRight, Check, Crown, X, Zap } from "lucide-react";
...
export function UpgradeBanner() {
	const { almostAtLimit, needsUpgrade, remaining, license, tierConfig } =
		useLicense();
	...
	return (
		...
		>
			<div
				className={`mt-0.5 ${needsUpgrade ? "text-red-500" : "text-amber-500"}`}
			>
				{needsUpgrade ? <AlertTriangle size={20} /> : <Zap size={20} />}
			</div>
			<div className="flex-1">
				<p
					className={`text-sm font-semibold ${needsUpgrade ? "text-red-800" : "text-amber-800"}`}
				>
					{needsUpgrade
				...
				<p
					className={`text-xs mt-1 ${needsUpgrade ? "text-red-600" : "text-amber-600"}`}
				>
					{needsUpgrade
...
export function UpgradeModal({
	...
	const upgradeTiers: LicenseTier[] = (
		["starter", "professional", "business"] as const
	).filter((t) => {
				...
				>
					...
					>
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-sm text-gray-500">
										Sie haben {license.bookingsUsed} von {license.bookingsLimit}{" "}
										Buchungen verwendet
									</p>
								</div>
							</div>
											...
											<div>
												...
												<p className="text-sm text-gray-500">
													{tier.bookingsLimit === -1
														...
														: tier.bookingsLimit}{" "}
													Buchungen
													{tier.bookingsLimit !== -1 ? "/Monat" : ""} · Bis{" "}
													{tier.staffLimit} Mitarbeiter
												</p>
											</div>
						...
						<p className="text-xs text-gray-400 text-center mt-4">
							Alle Preise zzgl. MwSt. · Jederzeit kündbar · 14 Tage kostenlos
							testen
						</p>
					</motion.div>
				</motion.div>
...
export function LicenseStatusWidget() {
	const { license, tierConfig, isUnlimited, remaining, usagePercent } =
		useLicense();
