				...
				<div className="card p-4">
					...
					<div className="space-y-3 ml-8">
						{settings.reminderTimes.map((time, i) => (
							// biome-ignore lint/suspicious/noArrayIndexKey: reminder times are addressed by index and the list does not reorder
							<div key={i} className="flex items-center gap-2">
								<select
									className="input-field text-sm w-auto"
									value={time}
									onChange={(e) =>
										handleReminderChange(i, Number(e.target.value))
									}
								>
