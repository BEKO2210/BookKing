...
export function AvailabilityEditor() {
  const { availability, upsertAvailability, deleteAvailability } = useAvailability();
              ...
              {avail && (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => handleUpdateTime(avail, 'startTime', e.target.value)}
                  ...
                  <input
                    type="time"
                    value={avail.endTime}
                    onChange={(e) => handleUpdateTime(avail, 'endTime', e.target.value)}
                    className="input-field py-1 px-2 w-auto text-sm"
                  />
                </div>
              )}
                ...
                {avail.breaks.map((brk, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm">
                    ...
                    <input
                      type="time"
                      value={brk.start}
                      onChange={(e) => handleUpdateBreak(avail, idx, 'start', e.target.value)}
                      className="input-field py-1 px-2 w-auto text-sm"
                    />
                    <span className="text-gray-400">–</span>
                    <input
                      type="time"
                      value={brk.end}
                      onChange={(e) => handleUpdateBreak(avail, idx, 'end', e.target.value)}
                      className="input-field py-1 px-2 w-auto text-sm"
                    />
                    <button
