...
import { getContrastColor, getInitials, stringToColor } from "@/lib/utils";
import { useBookingStore } from "@/store/booking-store";
import { motion } from "framer-motion";
import { ArrowLeft, ChevronRight } from "lucide-react";

export function StaffSelector() {
	const { selectStaff, nextStep, prevStep, selectedService } =
		useBookingStore();
	...
	const availableStaff = selectedService
		? DEMO_STAFF.filter(
				(s) =>
					...
					s.serviceIds.includes(selectedService.id),
			)
		: DEMO_STAFF;

	const handleSelect = (staff: (typeof DEMO_STAFF)[number] | null) => {
		selectStaff(staff ? { ...staff, avatar: undefined } : null);
		nextStep(); // → datetime
	};
