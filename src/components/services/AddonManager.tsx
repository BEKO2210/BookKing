...
export function AddonManager({ addons, onChange }: AddonManagerProps) {
	const handleAdd = () => {
		onChange([
			...addons,
			{ id: generateId(), name: "", duration: 15, price: 0 },
		]);
	};
				...
				{addons.map((addon) => (
					<div key={addon.id} className="flex items-center gap-2">
						...
						<input
							...
							onChange={(e) =>
								handleUpdate(addon.id, "duration", Number(e.target.value))
							}
							placeholder="Min."
							min={0}
						/>
						<input
							...
							value={addon.price}
							onChange={(e) =>
								handleUpdate(addon.id, "price", Number(e.target.value))
							}
							placeholder="€"
							min={0}
							step={0.01}
