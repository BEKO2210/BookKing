    ...
    <section className="relative pt-32 pb-20 lg:pt-44 lg:pb-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-4xl mx-auto">
          ...
          >
            Das intelligente Buchungssystem das sich mit Calendly und SimplyBook messen kann — als installierbare Progressive Web App. Vollständig. Bezahlbar. Deins.
          </motion.p>
    ...
    <div className="flex h-[420px] sm:h-[500px]">
      ...
      <div className="flex-1 p-4 sm:p-6 overflow-hidden">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {[
            { label: 'Heute', value: '8', sub: 'Termine', color: 'blue', icon: Calendar },
            { label: 'Umsatz', value: '2.340 €', sub: 'diesen Monat', color: 'green', icon: TrendingUp },
            { label: 'Auslastung', value: '87%', sub: 'diesen Monat', color: 'amber', icon: BarChart3 },
            { label: 'Kunden', value: '156', sub: 'Gesamt', color: 'purple', icon: Users },
          ].map((kpi) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4">
          <p className="text-xs font-medium text-gray-500 mb-3">Umsatz (letzte 30 Tage)</p>
          <div className="flex items-end gap-1 h-20">
            {[30, 45, 35, 60, 50, 75, 65, 80, 70, 90, 85, 95, 80, 100, 90, 88, 92, 78, 85, 95, 88, 100, 92, 85, 90, 95, 100, 92, 88, 96].map(
              (h, i) => (
                <div
                  key={i}
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-xs font-medium text-gray-500 mb-2">Heutige Termine</p>
          {[
            { time: '09:00', name: 'Haarschnitt', client: 'Anna M.', color: '#3b82f6' },
            { time: '10:30', name: 'Färben', client: 'Lisa K.', color: '#8b5cf6' },
            { time: '12:00', name: 'Styling', client: 'Tom S.', color: '#ec4899' },
          ].map((t) => (
            <div key={t.time} className="flex items-center gap-2 py-1.5">
              <div className="w-0.5 h-6 rounded-full" style={{ backgroundColor: t.color }} />
              <span className="text-[11px] text-gray-400 w-10">{t.time}</span>
              <span className="text-[11px] font-medium text-gray-800">{t.name}</span>
              <span className="text-[11px] text-gray-400 ml-auto">{t.client}</span>
            </div>
          ))}
        </div>
      </div>
...
function LogoCloud() {
  const industries = [
    'Friseure', 'Coaching', 'Arztpraxen', 'Fitness', 'Beauty', 'Beratung',
    'Massage', 'Tattoo', 'Fahrschulen', 'Nachhilfe', 'Therapeuten', 'Agenturen',
  ];
    ...
    <section className="py-12 border-y border-gray-100 bg-gray-50/50">
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
          {industries.map((name) => (
            <span key={name} className="text-sm font-medium text-gray-400 hover:text-gray-600 transition-colors">
              {name}
            </span>
          ))}
        </div>
  ...
  const steps = [
    {
      ...
      visual: (
        <div className="space-y-2">
          {[
            { name: 'Haarschnitt', dur: '45 Min.', price: '35 €', color: '#3b82f6' },
            { name: 'Färben & Styling', dur: '120 Min.', price: '89 €', color: '#8b5cf6' },
            { name: 'Bartpflege', dur: '30 Min.', price: '25 €', color: '#ec4899' },
          ].map((s) => (
            <div key={s.name} className="bg-white rounded-xl border border-gray-100 p-3 flex items-center gap-3 hover:border-blue-200 transition-colors cursor-pointer">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: s.color }} />
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900">{s.name}</p>
                <p className="text-[11px] text-gray-400">{s.dur}</p>
              </div>
              <span className="text-sm font-semibold text-gray-700">{s.price}</span>
            </div>
          ))}
        </div>
      ),
      ...
      visual: (
        <div className="space-y-2.5">
          <div>
            ...
            <div className="bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-700">Max Mustermann</div>
          </div>
          <div>
            <label className="text-[10px] text-gray-400 mb-0.5 block">E-Mail</label>
            <div className="bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-700">max@email.de</div>
          </div>
          <div>
            <label className="text-[10px] text-gray-400 mb-0.5 block">Telefon</label>
            <div className="bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-700">+49 170 1234567</div>
          </div>
        </div>
      ),
    },
    {
      ...
      visual: (
        <div className="text-center py-4">
          ...
          <p className="text-sm font-semibold text-gray-900 mb-1">Termin bestätigt!</p>
    ...
    <section id="demo" className="py-24 lg:py-32 bg-gray-50" ref={ref}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-12">
          ...
          >
            Live ausprobieren
            <MousePointer size={18} className="group-hover:translate-x-0.5 group-hover:translate-y-0.5 transition-transform" />
          </button>
        </div>
      </div>
    </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="mt-4 text-lg text-gray-500">
              Alles was Sie brauchen um Ihre Termine, Kunden und Umsätze zu verwalten — in einem
              elegant designten Dashboard.
            </p>
          ...
          >
              ...
              <div className="w-full h-full bg-white rounded-[2.25rem] overflow-hidden">
                ...
                <div className="p-4 pt-8">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-xs font-bold text-gray-900">BookKing</p>
                  ...
                  <p className="text-[10px] font-semibold text-gray-500 mb-2">NÄCHSTER TERMIN</p>
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-3 text-white mb-4">
                    ...
                    <p className="text-[11px] opacity-80">09:00 — Anna Müller</p>
                    <p className="text-[10px] opacity-60 mt-1">in 15 Minuten</p>
                  </div>

                  <p className="text-[10px] font-semibold text-gray-500 mb-2">WEITERE TERMINE</p>
                  {[
                    { t: '10:30', n: 'Färben', c: 'Lisa K.' },
                    { t: '12:00', n: 'Styling', c: 'Tom S.' },
                    { t: '14:00', n: 'Bartpflege', c: 'Jan R.' },
                  ].map((item) => (
                    <div key={item.t} className="flex items-center gap-2 py-1.5 border-b border-gray-50">
                      <span className="text-[10px] text-gray-400 w-8">{item.t}</span>
                      <span className="text-[11px] font-medium text-gray-800 flex-1">{item.n}</span>
                      <span className="text-[10px] text-gray-400">{item.c}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around py-2 px-1 rounded-b-[2.25rem]">
                  {[Layout, Calendar, Users, BarChart3].map((Icon, i) => (
                    <div key={i} className={`p-1.5 ${i === 0 ? 'text-blue-600' : 'text-gray-300'}`}>
                      <Icon size={16} />
                    </div>
                  ))}
                </div>
            ...
            <div className="absolute -left-4 top-20 bg-white rounded-xl shadow-lg border border-gray-100 p-3 flex items-center gap-2">
              ...
              <div>
                <p className="text-[11px] font-semibold text-gray-900">Neue Buchung!</p>
                <p className="text-[10px] text-gray-400">Anna M. · Haarschnitt</p>
              </div>
            </div>
            ...
            <div className="absolute -right-4 bottom-32 bg-white rounded-xl shadow-lg border border-gray-100 p-3 flex items-center gap-2">
              ...
              <div>
                <p className="text-[11px] font-semibold text-gray-900">PWA installiert</p>
    ...
    <section className="py-16 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              <p className="text-4xl lg:text-5xl font-extrabold text-white">{s.value}</p>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-12 lg:p-16 shadow-xl shadow-blue-500/15">
          ...
          <p className="mt-4 text-lg text-blue-100 max-w-xl mx-auto">
            Starten Sie in unter 5 Minuten. Keine Kreditkarte nötig.
            Sofort einsatzbereit.
          </p>
