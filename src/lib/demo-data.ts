...
export const DEMO_SERVICES: Service[] = [
	...
	},
	{
		...
		capacity: 1,
		category: "Haarschnitte",
		addons: [{ id: "addon-3", name: "Bartpflege", duration: 15, price: 12 }],
		isActive: true,
		sortOrder: 1,
		color: "#6366f1",
	},
...
function generatePastBookings(): Booking[] {
	...
	for (let d = 1; d <= 30; d++) {
		...
		for (let b = 0; b < count && b < times.length; b++) {
			const svcId = serviceIds[b % serviceIds.length] ?? "svc-1";
			const status =
				d === 8 && b === 0
					? ("no-show" as const)
					: d === 12 && b === 1
			...
			bookings.push({
				...
				customerId: customerIds[b % customerIds.length] ?? "cust-1",
				date,
				startTime: times[b] ?? "09:00",
				endTime: (times[b] ?? "09:00").replace(":00", ":45"),
				status,
				addons: [],
