...
export function getHolidays(
	...
	holidays.push(
		...
		{
			date: offsetDate(easterDate, 39),
			name: "Christi Himmelfahrt",
			type: "public",
		},
		{
			date: offsetDate(easterDate, 49),
			name: "Pfingstsonntag",
			type: "public",
		},
	...
	if (region === "DE") {
		...
		if (state) {
			const corpusChristiStates: GermanState[] = [
				"BW",
				"BY",
				"HE",
				"NW",
			...
			const reformationStates: GermanState[] = [
				"BB",
				"HB",
				"HH",
				"MV",
...
export function isHoliday(
	...
	const year = Number.parseInt(dateStr.split("-")[0] ?? "2026", 10);
