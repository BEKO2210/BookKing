...
export class BookKingDB extends Dexie {
	...
	constructor() {
		...
		this.version(1).stores({
			...
			bookings:
				"id, providerId, serviceId, customerId, date, status, createdAt",
...
function convertValue(
	value: unknown,
	converter: (obj: Record<string, unknown>) => Record<string, unknown>,
): unknown {
	if (value === null || typeof value !== "object") return value;
	if (Array.isArray(value))
		return value.map((item) => convertValue(item, converter));
